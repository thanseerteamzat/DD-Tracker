...
import { adjdbaEntry, adjdbaList } from '../models/adjlastid';
import { Center } from '../models/Center';

@Component({
  selector: 'app-adjdba-details',
  templateUrl: './adjdba-details.component.html',
  styleUrls: ['./adjdba-details.component.css']
})
export class AdjdbaDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    let dlRef = db.object('adjdbaEntry');
    dlRef.snapshotChanges().subscribe(action => {
      var quatationsList = action.payload.val();
      let obj = Common.snapshotToArray(action.payload);
      // this.dbaList = [];
      obj.forEach(element => {
        let ddListItem = new adjdbaList();

        let obj: adjdbaEntry = JSON.parse(element);
